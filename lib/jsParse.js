...
function parse(opt, result, map) {
	...
	if (isMoudle()) {
		var isAddWrapper;
		if (_.extname(src) !== 'html' && _.extname(src) !== 'htm'){
			isAddWrapper = true;
		} else {
			isAddWrapper = alp.config.get('isJswrapperInHtml');
		}

		//isAddWrapper = alp.config.get('isJswrapperInHtml') || (_.extname(src) !== 'html' && _.extname(src) !== 'htm');
