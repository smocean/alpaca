...
module.exports = function(src, content, isParse) {
	...
	isParse = _.is(isParse, 'undefined') ? true : isParse;
			...
			if (nType === 'callexpression' && node.callee.name === 'require') {
				...
				if (!_.isJsFile(key)) {
					if (isReadCss || _.extname(key) !== 'css') {
						...
						alp.log.error('在配置文件readcss = false的情况下，不能将require("' + node.arguments[0].value + '")做赋值运算,在' + jsAbsUrl + '文件中');
					}
