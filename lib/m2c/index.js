...
function buildConfig(opts) {
	...
	return {
		...
		isCanReadFileContent: function(path) {

			var extname = _.path.extname(path).replace('.', '');

			if (extname.toLocaleLowerCase() === 'js') {

				return false;
			} else {

				return _config.rdExt.indexOf(extname) >= 0 || _config.rdExt.indexOf('.' + extname) >= 0;
...
function getdeps(url, result) {
	...
	var absUrl, content, isJsFile, isCanReadContent,
	...
	isCanReadContent = config.isCanReadFileContent(absUrl);


	if (!(url in result) && !isCanReadContent) {
		result[url] = {};
	}
	...
	} else if (_.isTextFile(url) && !isCanReadContent) {
...
function doWithResult(result) {
	...
	for (key in result) {
		...
		obj.content = _.escodegen.generate(obj.ast);
...
function build(data, jsAbsUrl) {
		...
		_.estraverse.replace(_ast, {
			enter: function(node, parent) {
				...
				if (nType === 'variabledeclaration') {
					checkExistNonJsFileAndReadFile(node, parent);
				}
			},
	...
	/**
	 * 对于非js文件同时又不是可读内容到代码中的文件，采用 xx= require();方式引用，抛出异常。
	
	 */
	function checkExistNonJsFileAndReadFile(node, parent) {
		var declarations = node.declarations,
			src, obj;
		for (var i = 0, obj; obj = declarations[i]; i++) {
			if (obj.init && obj.init.type.toLocaleLowerCase() === 'callexpression' && obj.init.callee.name == 'require') {
				src = _.path.relative(base, _.path.resolve(dir, obj.init.arguments[0].value));


				if (_.isTextFile(src) && !_.isJsFile(src) && !config.isCanReadFileContent(src)) {

					throw new Error('不是js文件，同时又不是可读内容的文本文件，不能使用' + obj.id.name + ' = ' + 'require("' + obj.init.arguments[0].value + '") in "' + jsAbsUrl + ' "');
				}
			}
