...
function txtParse(opt) {
	var base = alp.config.get('base'),
		...
		cnt,
		content,
		useBase = alp.config.get('useBaseInFile');
	...
	_content = content.replace(regExp, function() {
		...
		if (!_.exists(absUrl)) {
			alp.log.error('unable to find [' + absUrl + ']:No such file in [' + src + ']');
		}
...
function _parse(opt, _result) {
	...
	for (var rel in _result) {
		obj = _result[rel];
		result[rel] = obj;
		result[rel].content = generateContent(obj.content, _.isLikeCss(rel), opt.minCallback);
...
module.exports = function(opt, settings, result) {
	alp.config.merge(settings || {});
	return parse(opt, result);
}
module.exports.deepParse = _parse;
